...
<head>
    ...
    <title>Expert Nursing Team - MMHC Admin</title>
    ...
    <div class="min-h-screen">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-blue-800 text-sm">Members appear in the "Meet Our Expert Nursing Team" section. Edit name, profile picture, title, rating, bio, and skills (comma-separated). Order below is the display sequence.</p>
                    </div>
                </div>
            </div>

            <div class="mb-6">
                <a href="{{ route('admin.featured-team.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">
                ...
                @forelse($items as $index => $item)
                    <div class="flex items-center gap-4 p-6 border-b border-gray-100 last:border-0">
                        <div class="flex flex-col gap-1">
                            <form action="{{ route('admin.featured-team.move-up', $item) }}" method="POST" class="inline">
                                @csrf
                            ...
                            <form action="{{ route('admin.featured-team.move-down', $item) }}" method="POST" class="inline">
                                @csrf
                        ...
                        <a href="{{ route('admin.featured-team.edit', $item) }}" class="px-3 py-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('admin.featured-team.destroy', $item) }}" method="POST" onsubmit="return confirm('Remove this team member?');">
                            @csrf
                            @method('DELETE')
            ...
            <div class="mt-6 text-center">
                <a href="{{ route('landing') }}#star-performers" target="_blank" class="inline-flex items-center text-blue-600 hover:text-blue-700 font-semibold">
