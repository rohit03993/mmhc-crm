    ...
    <div class="card-body p-3">
        <div class="d-flex flex-column flex-md-row justify-content-between gap-2 mb-2">
            <div class="d-flex align-items-start gap-2">
                @include('community::feed.partials.author-avatar', ['person' => $post->user, 'size' => 30])
                <div>
                    <h6 class="mb-1 fw-semibold">{{ $post->user->name }}</h6>
                    <small class="text-muted">
                        {{ ucfirst(str_replace('_', ' ', $post->user->role)) }}
                        @if($post->user->unique_id)
                            · {{ $post->user->unique_id }}
                        @endif
        ...
        <div class="reaction-actions mb-2">
            <div class="reaction-actions__inner">
                @php
                ...
                @foreach($reactionOptions as $type => $option)
                    <form method="POST" action="{{ route('community.reactions.react', $post) }}" class="reaction-form">
                        @csrf
                        <input type="hidden" name="page" value="{{ request('page', 1) }}">
                        <input type="hidden" name="reaction_type" value="{{ $type }}">
                        <button type="submit" class="btn btn-sm rounded-pill px-3 reaction-form-btn {{ $myReaction === $type ? 'btn-primary' : 'btn-outline-primary' }}">
                            <i class="fas {{ $option['icon'] }} me-1"></i>{{ $option['label'] }} ({{ $reactionCounts[$type] ?? 0 }})
                        </button>
                    </form>
                @endforeach
                <div class="reaction-actions__meta">
                <span class="badge rounded-pill text-bg-light px-2 py-2 align-self-center"><i class="fas fa-comment me-1"></i>{{ $post->comments_count }} comments</span>
                @if($post->post_type === 'event')
                    @php $myInterest = $post->eventInterests->firstWhere('user_id', auth()->id()); @endphp
                    <form method="POST" action="{{ route('community.events.interest', $post) }}" class="reaction-form">
                        @csrf
                        <input type="hidden" name="page" value="{{ request('page', 1) }}">
                        <input type="hidden" name="status" value="interested">
                        <button type="submit" class="btn btn-sm rounded-pill px-3 reaction-form-btn {{ optional($myInterest)->status === 'interested' ? 'btn-warning' : 'btn-outline-warning' }}">Interested</button>
                    </form>
                    <form method="POST" action="{{ route('community.events.interest', $post) }}" class="reaction-form">
                        @csrf
                        <input type="hidden" name="page" value="{{ request('page', 1) }}">
                        <input type="hidden" name="status" value="going">
                        <button type="submit" class="btn btn-sm rounded-pill px-3 reaction-form-btn {{ optional($myInterest)->status === 'going' ? 'btn-success' : 'btn-outline-success' }}">Going</button>
                    </form>
                    @if($myInterest)
                        <form method="POST" action="{{ route('community.events.interest', $post) }}" class="reaction-form">
                            @csrf
                            ...
                            <button type="submit" class="btn btn-sm btn-outline-secondary rounded-pill px-3 reaction-form-btn">Clear</button>
                        </form>
                    @endif
                @endif
                </div>
            ...
            <div class="collapse" id="commentsBlock{{ $post->id }}">
                @forelse($post->comments->whereNull('parent_id') as $comment)
                    <div class="mb-2 p-2 bg-light-subtle rounded-3 border">
                        <div class="d-flex justify-content-between align-items-start gap-2">
                            <div class="d-flex align-items-start gap-2">
                                @include('community::feed.partials.author-avatar', ['person' => $comment->user, 'size' => 24])
                ...
                <form method="POST" action="{{ route('community.comments.store', $post) }}">
                    ...
                    <div class="input-group">
                        <input type="text" name="content" class="form-control" placeholder="Write a comment..." required maxlength="1000" autocomplete="off">
