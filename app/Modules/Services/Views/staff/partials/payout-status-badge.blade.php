...
@elseif($status === StaffEarningStatusResolver::PENDING_PATIENT_OTP)
    <span class="badge rounded-pill bg-secondary" title="SMS OTP on the patient mobile entered on the form">{{ $label }}</span>
@elseif($status === StaffEarningStatusResolver::PENDING_REFERRAL_OTP)
    <span class="badge rounded-pill bg-secondary" title="SMS OTP on the referred nurse/caregiver mobile">{{ $label }}</span>
