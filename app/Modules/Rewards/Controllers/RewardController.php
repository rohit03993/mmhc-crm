...
class RewardController extends Controller
{
    ...
    public function index()
    {
        ...
        $payoutService = app(\App\Modules\Payments\Services\StaffPayoutService::class);
        ...
        return view('rewards::rewards.index', [
            ...
            'staffNeedsMobileVerification' => $staffNeedsMobileVerification,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($user->staffMustVerifyMobileBeforeRewards()) {
            return redirect()->back()
                ->withErrors(['patient_phone' => 'Verify your account mobile number under Profile before submitting patient rewards.'])
                ->withInput();
        }
        ...
        $sendOtp = $this->rewardService->sendVerificationOtp($reward);
    ...
    public function sendOtp(CaregiverReward $reward)
    {
        ...
        $res = $this->rewardService->sendVerificationOtp($reward);
    ...
    public function resendOtpFromBanner(Request $request, CaregiverReward $reward)
    {
        ...
        if ($user->staffMustVerifyMobileBeforeRewards()) {
            return redirect()->back()
                ->with('error', 'Verify your account mobile under Profile before using patient rewards.');
        }
        if ($user->hasPendingMobileContactVerification()) {
            return redirect()->back()
    ...
    public function adminIndex(Request $request)
    {
        ...
        $totalPoints = (int) CaregiverReward::verified()->sum('reward_points');
        $totalAmount = (float) CaregiverReward::verified()->sum('reward_amount');
    ...
    public function adminStaffDetail(User $staff)
    {
        ...
        $rewardStats = [
            ...
            'verified_count' => (int) CaregiverReward::where('user_id', $uid)->verified()->count(),
            'pending_count' => (int) CaregiverReward::where('user_id', $uid)
                ->where(function ($q) {
                    $q->where('verification_status', 'pending')
                        ->orWhereNull('verification_otp_sent_at');
                })
                ->count(),
            'total_points' => (int) CaregiverReward::where('user_id', $uid)->verified()->sum('reward_points'),
            'total_amount' => (float) CaregiverReward::where('user_id', $uid)->verified()->sum('reward_amount'),
            'credited_points' => (int) ($staff->reward_points ?? 0),
        ];
    ...
    protected function buildStaffPatientRewardLeaderboard(): Collection
    {
        return User::query()
            ...
            ->withSum(['caregiverRewards as patient_reward_points' => function ($q) {
                $q->verified();
            }], 'reward_points')
            ->withSum(['caregiverRewards as patient_reward_amount' => function ($q) {
                $q->verified();
            }], 'reward_amount')
